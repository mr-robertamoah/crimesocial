export function DP(
    {username = null, size = 100, className = '', avatarUrl= '', alt = 'profile pic'}: 
    {username?: string|null, size: number, className?: string, avatarUrl?: string; alt?: string}
) {
    return (
        <div className={`${className} relative h-[${size}px] w-[${size}px] bg-blue-600 rounded-full flex justify-center items-center`}>
            <div className={`h-[${size - 10}px] w-[${size - 10}px] bg-blue-200 rounded-full flex z-[2] justify-center items-center`}>
                <div className={`bg-gray-400 h-[${size - 15}px] w-[${size - 15}px] text-xs text-center flex justify-center items-center rounded-full`}>
                    {avatarUrl ? 
